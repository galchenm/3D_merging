...
def apply_geometry_from_file(data_as_slab, geometry_filename):
    ...
    img_shape = get_image_shape(geometry_filename)
    #print(img_shape)
    x, y, slab_shape = pixel_maps_for_image_view(geometry_filename)
    #print(slab_shape)
    im_out = numpy.zeros(img_shape, dtype=data_as_slab.dtype)
    #im_out = numpy.ravel(max_in(y),max_in(x))
    im_out[y,x] = data_as_slab.ravel()
    return im_out
    # print(max_in(y),max_in(x))
    # return 0
